...
class PagedAttentionOp(nn.Module):
  ...
  def kv_pages(self, model_mode: str):
  ...
  @nn.compact
  def __call__(self, query: Array, key: Array, value: Array, decoder_segment_ids: Array, model_mode: str) -> Array:

    key_pages_var, value_pages_var = self.kv_pages(model_mode)
    self.update(key_pages_var, value_pages_var, key, value, model_mode)
    ...
    if model_mode == common_types.MODEL_MODE_AUTOREGRESSIVE:
      ...
      result = paged_attention(
        ...
        lengths=self.page_manager.page_state.seq_lengths,
        page_indices=self.page_manager.page_state.seq_page_idx_mappings,
        pages_per_compute_block=self.pages_per_compute_block
      )
      return jnp.expand_dims(result, axis=1)

  def update(self, key_pages_var: nn.Variable, value_pages_var: nn.Variable, key: Array, value: Array, model_mode: str) -> None:
    if model_mode == common_types.MODEL_MODE_PREFILL:
      self.update_prefill_step_pages(key_pages_var, value_pages_var, key, value)
    elif model_mode == common_types.MODEL_MODE_AUTOREGRESSIVE:
      self.update_decode_step_pages(key_pages_var, value_pages_var, key, value)

  def update_prefill_step_pages(self, key_pages_var: nn.Variable, value_pages_var: nn.Variable, key: Array, value: Array) -> None:
  ...
  def update_decode_step_pages(
      self, key_pages_var: nn.Variable, value_pages_var: nn.Variable, key: Array, value: Array
  ) -> None:
    ...
    page_state = self.page_manager.page_state
    ...
    key_pages_var.value, _, _ = jax.lax.fori_loop(0, self.page_manager.slots, _update_pages, (key_pages_var.value, keys, seq_page_indices))
    value_pages_var.value, _, _ = jax.lax.fori_loop(0, self.page_manager.slots, _update_pages, (value_pages_var.value, values, seq_page_indices))
    key_pages_var.value = nn.with_logical_constraint(key_pages_var.value, self.kv_pages_axis_names)
    value_pages_var.value = nn.with_logical_constraint(value_pages_var.value, self.kv_pages_axis_names)
...
class Attention(nn.Module):
  ...
  def __call__(
    ...
    if self.attention_kernel == "paged":
